            ...
            case USB_CMD_GET_FW_INFO:
                 if(Get_Device_Status()==DEVICE_DFU_MODE)  //检查状态是不是DFU模式，步骤为获取长度
                 {
                    ...
                    else if(dfu_step== GET_MCU_FW_INFO)
                    {
                         ...
                         usb_write (USB_CMD_GET_RAW_DATA,&dfu_folow_num, 1);  //发送请求数据
                        // HAL_Delay(10);   
                    ...
                    else if(dfu_step==DOWNLOAD_MCU_FW)
                    {
                        ...
                        else
                        {
                            ...
                            else  //下载完毕
                            {
                                ...
                                usb_write (USB_CMD_GET_CHEKSUM,&dfu_folow_num, 1);  //发送请求数据 
...
//                        update_dis_online_page(payload[0],payload[1]);
        ...
        case USB_CMD_SYNC_FIRST_HEADER:  //开始搜索一个笔记的第一个笔记头
        {
            if(DEVICE_SYNC_MODE == Get_Device_Status())
            {     
                //report note header
        ...
        case  USB_CMD_SYNC_IS_END :   //如果相同笔记ID的块返回这个笔记下一个块的头部 如果搜索不到了 就将之前这个笔记ID所有的头部清除掉 然后返回2说明这个笔记传输完可以下一个笔记了
        {
             if((DEVICE_SYNC_MODE == Get_Device_Status()) && (0 == note_upload_flag))
            {
                ...
                Update_device_info((uint8_t *)&g_device_info, sizeof(st_device_data_storage));						       
                usb_write( USB_CMD_SYNC_IS_END , &end_content, 1);  //返回  0
        ...
        {    
            ...
            if(res)
            {
                ...
                usb_host_report_error(ERROR_TEST_FAILED); //返回错误  
                qiut_test_ble_net();
...
//               change_screen(TEST_NRF_SCREEN); //进入特殊界面
//            }   
        ...
        case  USB_CMD_TEST_MODE_QUIT:
          ...
          qiut_test_ble_net();
         // quit_test_2g4_net();
          change_screen(MAIN_SCREEN); //进入特殊界面
