...
void SPI_Write_Command(uint8_t cmd)
{
    ...
    HAL_SPI_Transmit(SPI_CONTROLER, &zero, 1, 10);
...
void OLED_Draw_BMP(uint8_t *Matrix_Buffer,uint8_t Start_Adr,uint8_t Use_Col_Num,uint8_t half)
{
    ...
    else
    {
        for(j=0;j<Use_Col_Num;j++)
        {
            OLED_GRAM[i][0]=*Matrix_Buffer;
            OLED_GRAM[i][1]=*(Matrix_Buffer+1);
            OLED_GRAM[i][2]=*(Matrix_Buffer+2);
            OLED_GRAM[i][3]=*(Matrix_Buffer+3);
            Matrix_Buffer+=4;
            i++;
        }
...
{      			    
    ...
    {   
        if(size==16)
        {
            temp=ASCII_Matrix_16[chr][t];	//调用1608字体
        }
        else if(size==20)
        {
           temp=ASCII_Matrix_20[chr][t];	//调用2010字体 
        }
        else 
        {
            return;								//没有的字库
...
//	SPI_Write_Command(129-Start_Adr);
      SPI_Write_Command(Start_Adr);
	SPI_Write_Command(Start_Adr+Use_Col_Num);
...
//	SPI_Write_Command(129-Start_Adr);
	...
	SPI_Write_Command(0);
	SPI_Write_Command(7);
	SPI_Write_Data(Matrix_Buffer,Use_Col_Num*8); // use 2 page to display ,so it need to *2
